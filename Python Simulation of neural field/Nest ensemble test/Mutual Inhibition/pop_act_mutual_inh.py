...
def pop_act(model,window,**kwargs):
    ...
    if model =='iaf':
       ...
       import iaf_ensemble_mutual_inh     as ie  
       pop_activity = ie.simulate(window,**kwvar) 
       return pop_activity
    elif model =='aeif':
       print 'run Gerstner model...'
       import Gerstner_ensemble_mutual_inh as Ge
       pop_activity = Ge.simulate(window,**kwvar)
       return pop_activity
    else :
...
if __name__ == '__main__':
    ...
    step = 10000.0
    for i in range(10) : 
        log = pop_act('iaf',10, pop=100, amp = step*(1+i),resl= 0.1, plot = False)
