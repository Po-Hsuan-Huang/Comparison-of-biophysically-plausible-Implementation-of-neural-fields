...
SIN units regard the spikes of NE and NI from one ensemble differently. 
SIN unit only responds to input from excitatory neourns inside ensembles, and
only inhibit excitatory neurons of the ensembles.That is, only excitatory neurons
have long distance connections reaching outside the ensembles.


Step_current_generator is used in this investigation.

Here we only record the spikes from excitatory neurons
...
J_ex_mean  =  0.1
g       = 2.0             #ratio between inh. and exc.   g = 2. is not enough to inhibit ensemble
...
f = 0.0025 # kKHz frequency of the injecting current 
...
nest.Connect(NE1, sup_NI1,conn_params, "inhibitory")
nest.Connect(NE1, sup_NI2,conn_params, "excitatory")
nest.Connect(NE2, sup_NI1,conn_params, "excitatory")
nest.Connect(NE2, sup_NI2,conn_params, "inhibitory")

''' Global inhibitoin of ensemble exerted by super neuron'''
nest.Connect(sup_NI1,NE1,conn_params, "inhibitory")
nest.Connect(sup_NI2,NE2,conn_params, "inhibitory")  
...
ax1=pl.subplot()
hist_1=ax1.hist(spikes1,bins = 100,alpha = 0.4,color='red',label = 'PSTH1') #return a tuple,[ value, edge  ]
pl.hold(True)
hist_2=ax1.hist(spikes2,bins = 100,alpha = 0.4,color='blue',label = 'PSTH2') #return a tuple,[ value, edge  ]
pl.title('PSTH of the two ensembles')
ax1.set_ylabel('# spikes')
ax1.set_xlim(0.,1000.0)
ax1.set_ylim(0., pl.amax(hist_1[0]))

ax2 = ax1.twinx()
ax2.plot(t1,v1,'r')
ax2.plot(t2,v2,'b')
ax2.plot(t1,[700.]*len(v1),'g')

ax2.set_ylabel('stimulation nA')
ax2.set_xlabel('time /ms')
ax2.set_xlim(0.,1000.0)
ax2.set_ylim(0.,2*pl.amax(v1) )
pl.legend()
pl.show()
