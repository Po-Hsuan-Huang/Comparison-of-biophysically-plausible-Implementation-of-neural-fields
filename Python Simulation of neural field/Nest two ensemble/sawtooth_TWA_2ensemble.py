...
epsilon = 0.1
CE    = int(epsilon*pop1_NE) # number of excitatory synapses per neuron
CI    = int(epsilon*pop1_NI) # number of inhibitory synapses per neuron  
...
def amp(amp,dt,f,phase):
    ...
    return amp*pl.absolute(signal.sawtooth(f*(2*pl.pi)*dt+phase+ pl.pi))-amp*pl.absolute(signal.sawtooth(f*(2*pl.pi)*dt+phase))+1600.0
...
''' neurons'''
NE1 = nest.Create("iaf_psc_alpha",pop1_NE)
NI1 = nest.Create("iaf_psc_alpha",pop1_NI)
...
NI2= nest.Create('iaf_psc_alpha',pop2_NI)
NE2= nest.Create('iaf_psc_alpha',pop2_NE)
...
nest.Connect(sup_NI1,NE1+NI1,conn_params, "inhibitory")
nest.Connect(sup_NI2,NE2+NI2,conn_params, "inhibitory")  
