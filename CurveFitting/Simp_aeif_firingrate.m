...
1. without subthreshold adaptation and exponential spike initiation
...
I3 = zeros(1,length(T));                      % applied current vector

v3 = zeros(1,length(T)); v3(1) = EL;  % membrane potential vector
w3 = zeros(1,length(T));             % adaptaion variable (~Ca-K current) 
...
FR3 = zeros(1, length(M));         % firing rate of simp_aeif neuron
...
for i = 1:length(M)   % apply voltage ramp
    ...
    counter = 0;
    FiringNeurons(:) = 0;
    v3 = EL*ones(1,length(T));           % membrane potential vector
    w3 = zeros(1,length(T));             % adaptaion variable (~Ca-K current) 
    g = zeros(1,length(T));             % total synaptic conductance variables vector
    E = zeros(1,length(T));             % synaptic reversal potentials vector
    I3(istart:iend) = M(i)*I0;           % initialise current pulse
    ...
    %%
    I3(istart:iend) = M(i)*I0*ones(1,iend-istart+1); %+y;           % initialise current pulse with white noise
        ...
        for t = Re_t:length(T)
            if ((sum(FiringNeurons(t-Re_t+1:t))) == 0) % refractory period : no membrane potential change with 2ms window of activation.
                ...
                % updated value v(i) is used in synaptic current to avoid instability
                 v3(t) = v3(t-1) + dt/Cm*(gL*(EL-v3(t-1)) - w3(t-1) + I3(t-1));
    
%                  w(t) = w(t-1) + dt/tauw*(a*(v(t-1)-EL) - w(t-1)); %update adaptation conductance
                  %compensation =    2.1659e-10;  % compensate for the absence of subthreshold adaptation.
                  w3(t) = w3(t-1) + dt/tauw*( - w3(t-1)) ;
               
                 if v3(t) >= Vt                                % if spike
                    FiringNeurons(1,t) = 1;
                    spike_num(i)=spike_num(i)+1;
                    v3(t) = Vp;                              % reset/update v and w   
                    w3(t) = w3(t-1) + b;
                end
            else
                v3(t) = Vr;
...
%% Plot Dynamics
figure(1)
plot(I_f*1e9,FR3,'g')% firing rate of simplified aeif neuron 2.0
hold on
plot(I_f*1e9,FR2,'m')% firing rate of simplified aeif neuron 1.0

plot(I_f*1e9,FR1,'b')% firing rate of aeif neuron
hold off
ylim([0,400])
...
legend('FR_simplified aeif','FR_aeif','aeif','Location','southeast')
title('f-I curve(aEIF)')
%%
% figure(2)
% %subplot(211)
% plot(T,1000*v3,'g') % firing rate of simplified aeif neuron2.0
% hold on
% plot(T,1000*v2,'m') % firing rate of simplified aeif neuron1.0
% 
...
% xlim([0.4,1.0])
% hold off
% legend('simp_aeif_2','simp_aeif_1','aeif')
% 
% subplot(212)
% plot(T,1000*w3,'g') % adaptive current of simplified neuron
% hold on 
